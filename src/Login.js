...
import { ActivityIndicator, StyleSheet, Text, View, TouchableOpacity, TextInput, Image, SafeAreaView } from 'react-native';
...
export default function Login({ navigation }) {
  ...
  const onHandleLogin = async () => {
    ...
    try {
      const response = await fetch('https://heavenly-onyx-bun.glitch.me/login', {
        method: 'POST',
        headers: {
      ...
      if (response.ok) {
        ...
        if (response.status === 401) {
          ...
          setErrorMessage('Incorrect cellphone or password. Please check your credentials.');
